...
public class CommonFunction {
	...
	private TimeoutDialogTryAgainListener tryAgainListener;
	
	public interface TimeoutDialogTryAgainListener{
		void tryAgain(AsyncTask<String, String, Long> conAsync);
	}
	
	public void setTimeoutDialogTryAgainListener(TimeoutDialogTryAgainListener listener){
	...
	public void showTimeoutDialog(final AsyncTask<String, String, Long> conAsync){
		((Activity) mctx).runOnUiThread(new Runnable() {
			
			@Override
			public void run() {
				AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(mctx);
				tryAgainListener = (TimeoutDialogTryAgainListener) mctx;
				alertDialogBuilder.setTitle("Connection Error");
				alertDialogBuilder.setMessage("Unable to connect with the server. Please chek your internet settings and try again");
				alertDialogBuilder.setPositiveButton("Try Again",new DialogInterface.OnClickListener() {

							@Override
							public void onClick(DialogInterface arg0,int arg1) {
								if(tryAgainListener != null){
									tryAgainListener.tryAgain(conAsync);
								}
							}
						});
	...
	public void callPostWebservice(String method,
		...
		AsyncTaskMain _asyncTask = new AsyncTaskMain(asyncListner, postMap,_activity);
		_asyncTask.setCalling_method(method);
		_asyncTask.execute("");
	}

	
	public void callPostWebservice(String methodOrUrl,HashMap<String, PostObject> postMap, AsyncTaskListener asyncListner,boolean isUrl) {
		AsyncTaskMain _asyncTask = new AsyncTaskMain(asyncListner, postMap,_activity);
		if(isUrl){
